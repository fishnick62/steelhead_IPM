...
flow_lag <- 0 #change lag to zero so that flow data aligns with fry stage
marine_lag <- 2
hrel_lag <- 2
...
We begin by getting the daily flow data from the US Geological Service [National Water Information System](http://waterdata.usgs.gov/nwis). We will use the direct link to the gage data from the Skagit River near Mount Vernon, WA (#12178000), beginning with the first year of fish data. #this gauge is for Newhalem not Mount Vernon??
...
flow_url <- paste0("https://waterdata.usgs.gov/nwis/dv",
                   "?cb_00060=on", #This is param code 00060 for daily mean stream flow
                   "&format=rdb",
                   "&site_no=",flow_site,
...
### This is now Spring/Summer Maximum from May through July
There is now no time lag to align with spawn and emergence year

```{r sum_flow, cache=TRUE}
## summer flows in year t
flow_sum <- subset(dat_flow, (month>=5 & month<=7)
                   & year >= yr_frst+flow_lag
                   & year <= yr_last-age_min+n_fore+flow_lag)
## change year index to brood year
